...
//  printSerial.print("data recv:"); printSerial.print(datals[1]);printSerial.print(";"); printSerial.println(datals[2]);
  //datals[1] = yaw ,datals[2] = pitch ,if receive datals[1] = 100 / 200 than yaw value = -200 / 200,if receive datals[2] = 100 / 200 than pitch value = -200 / 200
  
  local_yaw = datals[1] ; local_pitch = datals[2];
  
  if (local_yaw > 100) {
    local_yaw = local_yaw - 100;
    yaw_value = 6 * local_yaw;
  }
  else if (0 < local_yaw) {
    yaw_value = -6 * local_yaw;
  }
  ...
  if (local_pitch > 100) {
    local_pitch = local_pitch - 100;
    pitch_value = 6 * local_pitch;
  }
  else if (0 < local_pitch) {
    pitch_value = -6 * local_pitch;
  }
  else {
    pitch_value = 0;
...
//    printSerial.print(pitch_value);printSerial.print(";");printSerial.println(alpha);    
    sbusChlData[3] = Revdata[4] + yaw_value ;   // yaw
    sbusChlData[1] = Revdata[2] + pitch_value *  -1;   // pitch
